...
public class JDSign_up extends JDialog implements ActionListener, KeyListener, FocusListener {
	...
	public JDSign_up() {
		...
		btnRegistrar.setBounds(156, 281, 125, 33);
		...
		btnLimpiar.setBounds(28, 281, 105, 33);
		...
		btnCerrar.setBounds(306, 281, 111, 33);
	...
	protected void actionPerformedBtnRegistrar(ActionEvent arg0) {
		...
		if (leerPwd().equals(leerPwdCofirm())) {
			if (leerUser().length()==0 || leerPwd().length()==0 ||
					...
					leerNomUser().length()==0 || leerApeUser().length()==0  ) {/*falta añadir el combo box que no sepueda seleccionar e indice 0*/
